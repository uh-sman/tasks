...
import SaleItem from "@/components/sale/Sale";
...
const CreateListing: React.FC = () => {
  ...
  const createListing = async (data:any) => {
    try {
      // const { } = data
      const response = await axios.post('http://localhost:4000/api/listing/:?userId=6597d2d6fb0269802d3a29c2', data)
      console.log('listing response',response)
      // 6597d2d6fb0269802d3a29c2
    } catch (error) {
      console.log(error)
    }
  }
    ...
    <div className="w-full mt-2">
      <div className="w-72 md:w-[420px] flex-shrink-0 lg:w-[1000px] min-w-sm bg-base-100">
        ...
        >
          <FormProvider {...methods}>
            {fields.map(({ title, subtitle, formName, subName }) => (
              <>
                ...
                {/* <SubItem title={title} subtitle={subtitle}/> */}
              </>
            ))}
          </FormProvider>
          <div className="flex flex-wrap mt-3 mb-6">
            <div className="w-full flex justify-between md:w-1/2 px-3 mb-6 md:mb-0">
              <h1 className="font-bold text-orange-300 text-xl">Amenities</h1>
            </div>
            <div className="w-full flex justify-between md:w-1/2 px-3 mb-6 md:mb-0">
              <h1 className="font-bold text-orange-300 text-xl">Documents</h1>
            </div>
          </div>
          <FormProvider {...methods}>
            {fields.map(({ title, subtitle }) => (
              <>
                <SubItem title={title} subtitle={subtitle} />
              </>
            ))}
          ...
          <FormProvider {...methods}>
            {fields.map(({ sale, payment }) => (
              <>
                <SaleItem title={sale} subtitle={payment} />
              </>
            ))}
          </FormProvider>
          {/* DO NOT EDIT THIS */}
