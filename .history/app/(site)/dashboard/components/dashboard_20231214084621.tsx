      ...
      <div className='lg:w-[315px] w-[200px] h-full min-h-screen fixed top-0 left-0 bg-[#F5E0B8]'>
                <div className='flex flex-col '>
...

                    {
                      routes.map((item) => (
                        <>
                        <Button handleClick={() => router.push(item.href)} className='text-left active:bg-white focus:bg-white text-[#118286] active:text-[#F48222] pl-[31px] py-[16px]' disabled={item.active} key={item.label} {...item}>
                          {item.label}
                        </Button>
                        </>
                      ))
