        ...
        <div className="flex min-h-screen justify-center bg-[#F5E0B8] p-0 w-1/2">
            ...
            <div className="w-72 md:w-[420px] lg:w-[560px] flex-shrink-0 min-w-sm bg-base-100">
              ...
              <form>
                ...
                <p className="font-semibold text-md">{isLogin ? 'Don`t have an account?' : 'Already have an acct'} <span onClick={ () => setIsLogin('LOGIN')} className="underline cursor-pointer">{isLogin ? "login" : 'signup'} <span onClick={ () => setIsLogin('SIGNUP')} className="underline cursor-pointer">{isLogin ? "signup" : 'login'}</span></p>
              </form>
            </div>
            ...
            <div className="w-72 md:w-[420px] lg:w-[560px] flex-shrink-0 min-w-sm bg-base-100">
              ...
              <form>
                <div className="form-control space-y-10">
                    ...
                    <Input id="name" className="block outline-none w-full rounded-md border-none py-[12px] pl-7 pr-2 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset  sm:text-sm sm:leading-6" placeholder='avatar' type='file'/> 
                    <Button className='bg-teal-500 w-full rounded-md py-4 font-medium text-white' disabled={isLoading} type="submit">
                        SIGNUP
                    </Button>
                </div>
                <p className="font-semibold text-md">{isLogin ? 'Don`t have an account?' : 'Already have an acct'} <span onClick={ () => setIsLogin('LOGIN')} className="underline cursor-pointer">{isLogin ? "login" : 'signup'}</span></p>
              </form>
            </div>
          </div>)}
        </div>
