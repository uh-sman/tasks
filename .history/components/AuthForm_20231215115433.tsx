        ...
        <div className="flex min-h-screen justify-center bg-[#F5E0B8] p-0 w-1/2">
            ...
            <div className="w-72 md:w-[420px] lg:w-[560px] flex-shrink-0 min-w-sm bg-base-100">
              ...
              <form onSubmit={handleSubmit}>
                ...
                {
                  formState === 'SIGNUP' ? <p className="font-semibold text-md">already have an account? <button onClick={() => setFormState('SIGNUP')} className="underline cursor-pointer">{formState ? "login" : 'signup'}</button></p> :
                  <p className="font-semibold text-md"> Don`t have an account? <button onClick={() => setFormState('LOGIN')} className="underline cursor-pointer">{formState ? "login" : 'signup'}</button></p>
                }
              </form>
            </div>
          </div>
