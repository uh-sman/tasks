    ...
    const createPost = async () => {
        ...
        if (avatar) {
          ...
          try {
            ...
            if (formState === 'SIGNUP') {
              const userRegistrationResponse = await axios.post('http://localhost:4000/api/users/register', requestBody);
            ...
            if (formState === 'LOGIN') {
              const userRegistrationResponse = await axios.post('http://localhost:4000/api/users/login', requestBody);
              console.log('User registration response:', userRegistrationResponse.data);
              router.push(`/dashboard/${userRegistrationResponse.data.userId}`)
              // Handle the response or update the state as needed
            }
