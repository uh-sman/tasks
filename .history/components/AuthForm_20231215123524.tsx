...
const AuthForm = () => {
    ...
    const handleFormUpload = async () => {
      if (formState === 'LOGIN') {
      const response = await axios.post('http://localhost:400/api/users/login', requestBody)
             console.log(response)
      }
      if (formState === 'SIGNUP') {
        const response = await axios.post('http://localhost:400/api/users/register', requestBody)
             console.log(response)
      }
    }
