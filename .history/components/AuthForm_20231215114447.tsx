        ...
        <div className="flex min-h-screen justify-center bg-[#F5E0B8] p-0 w-1/2">
            ...
            <div className="w-72 md:w-[420px] lg:w-[560px] flex-shrink-0 min-w-sm bg-base-100">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <p className="font-semibold text-md">{formState === 'SIGNUP' ?  'already have an account?' : 'Don`t have an account?' } <button onClick={() => setFormState('SIGNUP' || 'LOGIN')} className="underline cursor-pointer">{formState ? "login" : 'signup'}</button></p>
              </form>
            </div>
          </div>
        </div>
